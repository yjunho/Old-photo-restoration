...
def main():
    ...
    if mode == 'single':
        opt['input_folder'] = './DIV2K/DIV2K_train_HR'
        opt['save_folder'] = './DIV2K/DIV2K_train_HR_sub'
        clear_directory(opt['save_folder'])
        opt['crop_sz'] = 480  # the size of each sub-image
        opt['step'] = 240  # step of the sliding crop window
        opt['thres_sz'] = 48  # size threshold
        extract_single(opt)
        ...
        extract_single(opt)
        print('process LR...')
        scale_ratio_list = [2, 4]  # Only process x2 and x4

        for scale_ratio in scale_ratio_list:
            print('process LR-X' + str(scale_ratio) + '...')
            LR_folder = './DIV2K/DIV2K_train_LR_bicubic/x' + str(scale_ratio)
            save_LR_folder = './DIV2K/DIV2K_train_LR_bicubic_sub/x' + str(scale_ratio) + '/'
            clear_directory(save_LR_folder)
            ...
            for path_GT, path_LR in zip(img_GT_list, img_LR_list):
                ...
                w_GT, h_GT = img_GT.size
                w_LR, h_LR = img_LR.size
                assert abs(w_GT / w_LR - scale_ratio) < 0.1, 'GT width [{:d}] is not {:d}X as LR width [{:d}] for {:s}.'.format(
                    w_GT, scale_ratio, w_LR, path_GT)
                assert abs(h_GT / h_LR - scale_ratio) < 0.1, 'GT height [{:d}] is not {:d}X as LR height [{:d}] for {:s}.'.format(
            ...
            GT_sub_images = _get_paths_from_images(save_GT_folder)
            LR_sub_images = _get_paths_from_images(save_LR_folder)
            print(f'GT sub-images: {len(GT_sub_images)}')
            print(f'LR sub-images: {len(LR_sub_images)}')
            assert len(GT_sub_images) == len(LR_sub_images), 'different length of save_GT_folder and save_LR_folder.'
    else:
        raise ValueError('Wrong mode.')
...
def worker(path, opt):
    ...
    print(f"Image {img_name} dimensions: {w}x{h}x{n_channels}")
    ...
    for x in h_space:
        for y in w_space:
            ...
            if n_channels == 2:
                ...
                crop_img = img[x:x + crop_sz, y:y + crop_sz, :]
            crop_img = np.ascontiguousarray(crop_img)
            save_path = osp.join(opt['save_folder'], img_name.replace('.png', f'_s{index:03d}.png'))
            print(f"Saving subimage {save_path}")
            success = cv2.imwrite(save_path, crop_img, [cv2.IMWRITE_PNG_COMPRESSION, opt['compression_level']])
            if not success:
                print(f"Failed to save subimage {save_path}")
...
#         opt['input_folder'] = './DIV2K/DIV2K_train_HR'
#         opt['save_folder'] = './DIV2K/DIV2K_train_HR_sub'
#         clear_directory(opt['save_folder'])  # Clear the save folder
#         opt['crop_sz'] = 32  # the size of each sub-image
#         opt['step'] = 16  # step of the sliding crop window
#         opt['thres_sz'] = 8  # size threshold
#         extract_single(opt)
#     elif mode == 'pair':
#         GT_folder = './DIV2K/DIV2K_train_HR'
#         save_GT_folder = './DIV2K/DIV2K_train_HR_sub/'

#         crop_sz = 32  # the size of each sub-image (GT)
#         step = 16  # step of the sliding crop window (GT)
#         thres_sz = 8  # size threshold
...
#         extract_single(opt)
#         print('process LR...')
#         scale_ratio_list = [2, 4]  # Process only x2 and x4

#         for scale_ratio in scale_ratio_list:
#             print('process LR-X' + str(scale_ratio) + '...')
#             LR_folder = './DIV2K/DIV2K_train_LR_bicubic/x' + str(scale_ratio)
#             save_LR_folder = './DIV2K/DIV2K_train_LR_bicubic_sub/x' + str(scale_ratio) + '/'
#             clear_directory(save_LR_folder)  # Clear the save folder
...
#             opt['thres_sz'] = thres_sz // scale_ratio
#             extract_single(opt)
#             assert len(_get_paths_from_images(save_GT_folder)) == len(_get_paths_from_images(save_LR_folder)), 'different length of save_GT_folder and save_LR_folder.'
#     else:
#         raise ValueError('Wrong mode.')
...
#     for dirpath, _, fnames in sorted(os.walk(path)):
#         for fname in sorted(fnames):
#             if fname.lower().endswith('.png'):  # Ensure only PNG files are considered
#                 img_path = os.path.join(dirpath, fname)
#                 images.append(img_path)
#     assert images, f'{path} has no valid image file'
#     return images
